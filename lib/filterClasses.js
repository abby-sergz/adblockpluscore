...
ActiveFilter.prototype = extend(Filter, {
  ...
  get domains()
  {
    ...
    if (this.domainSource)
    {
      ...
      if (list.length == 1 && list[0][0] != "~")
      {
        ...
        let domain = Filter.knownDomains.get(list[0]);
        if (!domain)
        {
          domain = list[0];
          Filter.knownDomains.set(domain, domain);
        }
      ...
      else
      {
        ...
        for (let i = 0; i < list.length; i++)
        {
          ...
          if (!domains)
            domains = new Map();
          let knownDomain = Filter.knownDomains.get(domain);
          if (!knownDomain)
          {
            knownDomain = domain;
            Filter.knownDomains.set(knownDomain, knownDomain);
          }
          domains.set(knownDomain, include);
        }
        if (domains)
  ...
  isActiveOnlyOnDomain(docDomain)
  {
    ...
    for (let [domain, included] of this.domains)
    {
      if (included && domain != docDomain)
      {
