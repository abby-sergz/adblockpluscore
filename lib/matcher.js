...
Matcher.prototype = {
  ...
  add(filter)
  {
    ...
    else
    {
      if (oldEntry.length == 1)
        this.filterByKeyword.set(keyword, new Set([oldEntry, filter]));
      else
        oldEntry.add(filter);
    }
  ...
  remove(filter)
  {
    ...
    else
    {
      list.delete(filter);
      if (list.size == 1)
        this.filterByKeyword.set(keyword, list.values().next().value);
    }
  ...
  findKeyword(filter)
  {
    ...
    for (let i = 0, l = candidates.length; i < l; i++)
    {
      ...
      let count = (hash.has(candidate) ? hash.get(candidate).length : 0);
  ...
  {
    if (this.keywordByFilter.has(filter.text))
      return this.keywordByFilter.get(filter.text);
    return null;
  },
  ...
  matchesAny(location, typeMask, docDomain, thirdParty, sitekey, specificOnly)
  {
    ...
    for (let i = 0, l = candidates.length; i < l; i++)
    {
      ...
      if (this.filterByKeyword.has(substr))
      {
...
CombinedMatcher.prototype =
{
                     ...
                     specificOnly)
  {
    ...
    {
      let substr = candidates[i];
      if (this.whitelist.filterByKeyword.has(substr))
      {
      ...
      if (this.blacklist.filterByKeyword.has(substr) && blacklistHit === null)
      {
